...
def main_worker(args):
    log_name='RegDB_s2'
    # 构建两个模型
    model0 = create_model(args)
    model1 = create_model(args)

    # ====================== visible to thermal ======================
    print("========== Test Visible to Thermal (Two Model Fusion) ==========")
    all_cmc_v2t = None
    all_mAP_v2t = 0.0
    all_mINP_v2t = 0.0
    for trial in range(1,11):
        ...
        # 加载两个模型权重 model_best_0 / model_best_1
        ckpt0 = load_checkpoint(osp.join(logs_dir_root, str(trial), 'model_best_0.pth.tar'))
        ckpt1 = load_checkpoint(osp.join(logs_dir_root, str(trial), 'model_best_1.pth.tar'))
        model0.load_state_dict(ckpt0['state_dict'])
        ...
        # 分别提取两套特征
        q_feat0 = extract_query_feat(model0, query_loader, nquery)
        q_feat1 = extract_query_feat(model1, query_loader, nquery)
        g_feat0 = extract_gall_feat(model0, gall_loader, ngall)
        g_feat1 = extract_gall_feat(model1, gall_loader, ngall)

        # 特征融合：平均
        q_final = (q_feat0 + q_feat1) / 2.0
        g_final = (g_feat0 + g_feat1) / 2.0

        # 距离矩阵
        distmat = np.matmul(q_final, g_final.T)
        cmc, mAP, mINP = eval_regdb(-distmat, query_label, gall_label)

        print('V2T Trial: {}'.format(trial))
        print(
            'FC Fusion: Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
    ...
    avg_cmc_v2t = all_cmc_v2t / 10
    avg_mAP_v2t = all_mAP_v2t / 10
    avg_mINP_v2t = all_mINP_v2t / 10
    print('\n===== V2T 10 Trials Average Fusion Result =====')
    print(
        'FC Fusion Avg: Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}\n'.format(
            avg_cmc_v2t[0], avg_cmc_v2t[4], avg_cmc_v2t[9], avg_cmc_v2t[19], avg_mAP_v2t, avg_mINP_v2t))

    # ====================== thermal to visible ======================
    print("========== Test Thermal to Visible (Two Model Fusion) ==========")
    all_cmc_t2v = None
    all_mAP_t2v = 0.0
    all_mINP_t2v = 0.0
    for trial in range(1,11):
    ...
    avg_cmc_t2v = all_cmc_t2v / 10
    avg_mAP_t2v = all_mAP_t2v / 10
    avg_mINP_t2v = all_mINP_t2v / 10
    print('\n===== T2V 10 Trials Average Fusion Result =====')
    print(
        'FC Fusion Avg: Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
            avg_cmc_t2v[0], avg_cmc_t2v[4], avg_cmc_t2v[9], avg_cmc_t2v[19], avg_mAP_t2v, avg_mINP_t2v))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Two Model Fusion Test for Visible-Infrared ReID")
    # data

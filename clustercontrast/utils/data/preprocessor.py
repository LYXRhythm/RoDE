...
class Preprocessor_gmm(Dataset):
    def __init__(self, dataset, root=None, transform=None):
        super(Preprocessor_gmm, self).__init__()
        self.dataset = dataset
        self.root = root
        self.transform = transform
    ...
    def _get_single_item(self, index):
        fname, pid, camid, gmm = self.dataset[index]
        fpath = fname
        if self.root is not None:
            fpath = osp.join(self.root, fname)
        ...
        return img, fname, pid, camid, index, gmm
    
class Preprocessor_color_gmm(Dataset):
    def __init__(self, dataset, root=None, transform=None,transform1=None):
        super(Preprocessor_color, self).__init__()
        self.dataset = dataset
        self.root = root
    ...
    def _get_single_item(self, index):
        ...
        if self.transform is not None:
            img = self.transform(img_ori)
            img1 = self.transform1(img_ori)
        return img, img1, fname, pid, camid, index, gmm

...
from ChannelAug import ChannelAdapGray, ChannelRandomErasing, ChannelExchange
...
def main_worker_stage1(args, log_s1_name):
    ...
    trial = args.trial
    stage1_logs_dir = osp.join(logs_dir_root, str(trial))
    ...
    for epoch in range(args.epochs):
        for ii in range(len(model_list)):
            with torch.no_grad():
                if epoch == 0:
                    ir_num_clusters = args.ir_num_cluster + args.over_cluster + 10
            ...
            if epoch>=0 and ( (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
                ...
                print("checkpoint: ", osp.join(stage1_logs_dir, 'checkpoint'+str(ii)+'.pth.tar'))
                print("model_best: ", osp.join(stage1_logs_dir, 'model_best_'+str(ii)+'.pth.tar'))
                
                save_checkpoint({
                    'state_dict': model_list[ii].state_dict(),
                    'epoch': epoch + 1,
                    'best_mAP': best_mAP[ii],
                }, is_best, fpath=osp.join(stage1_logs_dir, 'checkpoint'+str(ii)+'.pth.tar'), model_best_path=osp.join(args.logs_dir, 'model_best_'+str(ii)+'.pth.tar'))
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    for epoch in range(args.epochs):
        ...
        for ii in range(len(model_list)):
            ...
            trainer_list[ii].train_gmm(epoch, epoch, train_loader_ir_easy, train_loader_rgb_easy, optimizer_list[ii],
            ...
            trainer_list[ii].train_gmm(epoch, epoch, train_loader_ir_diff, train_loader_rgb_diff, optimizer_list[ii],
            ...
            trainer_list[ii].train_gmm(epoch, epoch+1, train_loader_ir_easy, train_loader_rgb_easy, optimizer_list[ii],
            ...
            trainer_list[ii].train_gmm(epoch, epoch+1, train_loader_ir_diff, train_loader_rgb_diff, optimizer_list[ii],
            ...
            if epoch>=0 and ( (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
                ...
                print("checkpoint: ", osp.join(stage2_logs_dir, 'checkpoint'+str(ii)+'.pth.tar'))
                print("model_best: ", osp.join(stage2_logs_dir, 'model_best_'+str(ii)+'.pth.tar'))
                
                save_checkpoint({
                    'state_dict': model_list[ii].state_dict(),
                    'epoch': epoch + 1,
                    'best_mAP': best_mAP[ii],
                }, is_best[ii], fpath=osp.join(stage2_logs_dir, 'checkpoint'+str(ii)+'.pth.tar'), model_best_path=osp.join(args.logs_dir, 'model_best_'+str(ii)+'.pth.tar'))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Unsupervised Visible-Infrared Person Re-Identification on RegDB Dataset")
    # data
    ...
    parser.add_argument('--seed', type=int, default=None,
                        help='seed must be set as "None" to ensure the different initization of two co-learning model')
